...
apiRouter.post("/generate", async (req: Request, res: Response) => {
  ...
  const extractMethod: "api" | "ocr" | "llm" = ["ocr", "llm"].includes(method) ? method : "api";
...
请生成结构化的中文对话文章：`;
    ...
    const stream = await withTimeout(
      model.stream(prompt),
      60000,
      "文章生成请求超时，请检查网络连接或 DASHSCOPE_API_KEY 配置"
    );
 ...
 */
apiRouter.post("/5w1h", async (req: Request, res: Response) => {
 ...
 * 获取任务的章节列表
 */
apiRouter.get("/sections/:taskId", (req: Request, res: Response) => {
