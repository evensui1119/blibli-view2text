 ...
 * 对帧图片进行 OCR 识别
 */
async function ocrFrames(framePaths, onProgress) {
    const results = [];
    // 创建 worker（支持简体中文）
    ...
    for (let i = 0; i < framePaths.length; i++) {
        ...
        try {
            const { data: { text }, } = await worker.recognize(framePaths[i]);
 ...
 * 视频文件 → 抽帧 → OCR → 去重 → 拼接文本
 */
export async function ocrExtractSubtitle(videoPath, onProgress) {
    // 创建临时帧目录
...
//# sourceMappingURL=ocr-subtitle.js.map